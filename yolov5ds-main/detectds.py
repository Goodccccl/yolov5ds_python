...
def run(weights=ROOT / 'yolov5s.pt',  # model.pt path(s)
    ...
    for i in range(clsnum + 1):
        # BGR model, the first is background.
        colorlist = [[255, 0, 0], [0, 255, 0], [160, 32, 240], [0, 0, 0]]
        ...
        for i, det in enumerate(pred):  # per image
            ...
            if save_img:
                if dataset.mode == 'image':
                    ...
                    im_vis = cv2.addWeighted(im0, 1, maskcolor, 1, 1)
                    cv2.imwrite("{}_yolods.png".format(save_path[:-4]), im_vis)
                    # cv2.imwrite("{}_yolods.png".format(save_path[:-4]), im0)
    ...
    print("total image : ", seen, ";  total time : ", totaltime, "s; average inference time pre image : ",
          totaltime / seen)


def parse_opt():
    parser = argparse.ArgumentParser()
    parser.add_argument('--weights', nargs='+', type=str, default=r'F:\Artificial_neural_Network\yolov5ds-main\yolov5ds-main\runs\train\left\weights\best.pt', help='model path(s)')
    parser.add_argument('--source', type=str, default=r'F:\Artificial_neural_Network\yolov5ds-main\yolov5ds-main\test', help='file/dir/URL/glob, 0 for webcam')
    parser.add_argument('--imgsz', '--img', '--img-size', nargs='+', type=int, default=[991], help='inference size h,w')
